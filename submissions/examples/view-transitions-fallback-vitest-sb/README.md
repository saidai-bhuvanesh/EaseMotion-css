...
This submission addresses issue #82050 by providing a self-contained demonstration and reusable artifact for **View Transitions Fallback Support Detection (Vitest unit)**, packaged inside `submissions/examples/view-transitions-fallback-vitest-sb/` to comply with the repository contribution guidelines.
...
- `style.css`
- `README.md`
- `view-transitions-fallback-vitest-sb.test.js`

## Usage

1. Open `demo.html` directly in a browser (no server needed).
2. Observe the effect and inspect `style.css` for the raw CSS.
3. Run `npx vitest run submissions/examples/<folder>/<file>.test.js` to execute the unit test.
